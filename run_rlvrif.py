...
from utils.groud_truth_utils import IFEvalVerifier
...
def get_main_args():
    ...
    parser.add_argument("--data_dirpath", type=str, default="datasets/rl/if")
    parser.add_argument("--output_dirpath", type=str, default="checkpoints/rl/rlvrif-qwen2.5-1.5b")
...
def main():
    ...
    if_ds = load_dataset("parquet", data_files=list_of_data_filepaths, split="train", num_proc=os.cpu_count() // 4)

    verifier = IFEvalVerifier()
    def make_if_reward(verifier):
        def if_reward(completions, ground_truth, **kwargs):
            ...
            return rewards
        return if_reward
    if_reward = make_if_reward(verifier)
