...
def main():
    list_of_allenai_data_filepaths = [
        str(filepath.absolute()) for filepath in Path("datasets/source/rl/ai2/code/dolci-rl-zero-code-7b").rglob("*.parquet")
    ]
    rl_ds = load_dataset("parquet", num_proc=os.cpu_count() // 4, data_files=list_of_allenai_data_filepaths, split="train")
    rl_ds = rl_ds.select_columns(["prompt", "ground_truth"])
    rl_ds = rl_ds.map(lambda x: {"prompt": [{"role": "user", "content": x["prompt"].replace("user: ", "")}], "ground_truth": x["ground_truth"][0]}, num_proc=os.cpu_count() // 4, remove_columns=["prompt", "ground_truth"])
    num_shards = calculate_proper_number_of_shards(rl_ds._estimate_nbytes())

    save_shard_ = partial(
        save_shard,
        num_shards=num_shards,
        output_dirpath="datasets/rl/code",
        dset=rl_ds,
    )
